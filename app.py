...
# --- 4. ADVANCED CUSTOM CSS & PRINT STYLING ---
st.markdown(
    """
    <style>
    .stApp {
...
# --- 7. HELPER BARU: KOMPONEN LIHAT, DOWNLOAD, DAN CETAK FILE ---
def render_file_action_buttons(file_path, label_prefix="Berkas"):
    """Menampilkan tombol Lihat dan Download untuk berkas fisik yang ada di server."""
    if file_path and file_path != "-" and os.path.exists(file_path):
        ...
        with col_btn2:
            st.caption(f"📁 Path: `{filename}`")
    elif file_path and file_path != "-":
        st.caption(f"⚠️ Berkas dicatat di `{file_path}`, namun fisik file tidak ditemukan di server.")
    else:
        st.caption("ℹ️ Belum ada berkas diunggah.")


def render_cetak_laporan_button(item):
    """Menampilkan format Resume Dokumen Siap Cetak (Printable HTML)."""
    timeline_html = "".join(
        [
            f"<li><b>[{t.get('waktu', '')}]</b> {t.get('pesan', '')}</li>"
...
# --- 8. FUNGSI CEK NOTIFIKASI PER DIVISI ---
def cek_notifikasi_user(role):
    ...
    if role == "Purchasing":
        ...
        pending_items = [
            x for x in db if x["status"] in ["2. Review BM", "5. Approval Akhir (BM & P3SRS)"]
        ]
    elif role == "Finance":
        ...
        for i, item_task in enumerate(pending_tasks):
            ...
            with btn_cols[col_idx]:
                if st.button(
                    ...
                    st.toast(f"🎯 Menuju berkas {item_task['nomor_opb']}", icon="⚡")
                    st.rerun()
    ...
    if role == "Engineering":
        ...
        with tab1:
            ...
            with st.form(key="form_opb_engineering", clear_on_submit=True):
                ...
                keterangan = st.text_area("Alasan Kebutuhan & Spesifikasi Detail", placeholder="Tuliskan alasan kebutuhan...")
        ...
        with tab2:
            ...
            for item in items:
                ...
                with st.expander(f"📦 {item['nomor_opb']} - {item['nama_barang']}", expanded=is_expanded):
                    ...
                    render_file_action_buttons(item.get("link_opb"), label_prefix="Lampiran OPB")
        ...
        with tab2:
            ...
            for item in items:
                ...
                with st.expander(f"📑 {item['nomor_opb']} - {item['nama_barang']}", expanded=is_expanded):
                    ...
                    if item.get("link_iom") != "-":
                        render_file_action_buttons(item.get("link_iom"), label_prefix="IOM Existing")

                    file_iom = st.file_uploader("Unggah / Ganti Draft Dokumen IOM", type=["pdf", "docx"], key=f"fiom_{item['id']}")
        ...
        with tab3:
            ...
            for item in items:
                ...
                with st.expander(f"💳 {item['nomor_opb']} - {item['nama_barang']}", expanded=is_expanded):
                    ...
                    render_file_action_buttons(item.get("link_iom"), label_prefix="IOM Approved")
        ...
        with tab1:
            ...
            for item in items:
                ...
                with st.expander(f"🧐 Review OPB: {item['nomor_opb']} - {item['nama_barang']}", expanded=is_expanded):
                    st.write(f"**Vendor:** {item['vendor']} | **Estimasi Harga:** Rp {item['harga_estimasi']:,}")
        ...
        with tab2:
            ...
            for item in items:
                ...
                with st.expander(f"📑 Approval IOM: {item['nomor_opb']} - {item['nama_barang']}", expanded=is_expanded):
                    ...
                    render_file_action_buttons(item.get("link_iom"), label_prefix="IOM Final")
    ...
    # 4. ROLE FINANCE
        ...
        for item in items:
            ...
            with st.expander(f"💵 Review IOM: {item['nomor_opb']} - {item['nama_barang']}", expanded=is_expanded):
                st.write(f"**Vendor:** {item['vendor']} | **Pengajuan Dana:** Rp {item['harga_estimasi']:,}")
    ...
    # 5. ROLE P3SRS
        ...
        for item in items:
            ...
            with st.expander(f"⚖️ Persetujuan Final: {item['nomor_opb']} - {item['nama_barang']}", expanded=is_expanded):
                st.write(f"**Vendor:** {item['vendor']} | **Total Anggaran:** Rp {item['harga_estimasi']:,}")
                ...
                col1, col2 = st.columns(2)
                with col1:
                    if st.button("✅ ACC & Instruksikan Pembelian", key=f"app_p3srs_{item['id']}", type="primary"):
                        item["status"] = "6. Pembelian Barang (Purchasing)"
                        catat_log(item, "P3SRS menyetujui IOM Final. Memerintahkan Purchasing melakukan pembelian.")
                        save_db(st.session_state["db_opb"])
                        st.session_state["target_focus_id"] = None
                        st.toast("🎉 IOM Disetujui P3SRS!", icon="✅")
                        st.rerun()
                with col2:
                    if st.button("❌ Minta Revisi", key=f"rej_p3srs_{item['id']}"):
