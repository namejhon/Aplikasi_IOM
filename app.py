...
def save_database(item_data, is_new=False):
    ...
    try:
        db_payload = {
            ...
            "foto_serah_terima_url": item_data.get("foto_serah_terima_url"),
            "foto_proses_pemasangan_url": item_data.get("foto_proses_pemasangan_url"),
            "foto_finishing_url": item_data.get("foto_finishing_url"),
...
if not st.session_state["logged_in"]:
    ...
    if total_opb > 0:
        ...
        with col_dash1:
            ...
            if selected_opb_label:
                ...
                # Tampilkan Preview Foto Dokumantasi BAST jika ada
                if selected_item.get("foto_serah_terima_url") or selected_item.get("foto_proses_pemasangan_url") or selected_item.get("foto_finishing_url"):
                    st.markdown("🖼️ **Dokumentasi BAST & Pemasangan:**")
                    f_cols = st.columns(3)
                    with f_cols[0]:
                        if selected_item.get("foto_serah_terima_url"):
                            st.image(selected_item["foto_serah_terima_url"], caption="Foto Serah Terima", use_container_width=True)
                    with f_cols[1]:
                        if selected_item.get("foto_proses_pemasangan_url"):
                            st.image(selected_item["foto_proses_pemasangan_url"], caption="Proses Pemasangan", use_container_width=True)
                    with f_cols[2]:
                        if selected_item.get("foto_finishing_url"):
                            st.image(selected_item["foto_finishing_url"], caption="Finishing / Terpasang", use_container_width=True)
    ...
    if role == "Engineering":
        ...
        with tab2:
            st.markdown("<div class='content-box'>", unsafe_allow_html=True)
            st.subheader("📦 Verifikasi Penerimaan Barang")
            ...
            for item in items:
                ...
                with st.expander(f"📦 {item.get('nomor_opb', '-')} - {item.get('nama_barang', '-')}", expanded=is_expanded):
                    ...
                    # Tampilkan dokumentasi yang diupload purchasing jika ada
                    if item.get("foto_serah_terima_url") or item.get("foto_proses_pemasangan_url") or item.get("foto_finishing_url"):
                        st.markdown("---")
                        st.markdown("📸 **Dokumentasi BAST dari Purchasing:**")
                        f_cols = st.columns(3)
                        with f_cols[0]:
                            if item.get("foto_serah_terima_url"):
                                st.image(item["foto_serah_terima_url"], caption="Foto Serah Terima", use_container_width=True)
                        with f_cols[1]:
                            if item.get("foto_proses_pemasangan_url"):
                                st.image(item["foto_proses_pemasangan_url"], caption="Proses Pemasangan", use_container_width=True)
                        with f_cols[2]:
                            if item.get("foto_finishing_url"):
                                st.image(item["foto_finishing_url"], caption="Finishing / Terpasang", use_container_width=True)

                    if st.button(f"✅ Konfirmasi Selesai & Tanda Tangan Digital Otomatis", type="primary", use_container_width=True):
                        div_pemohon = item.get('divisi', 'Engineering')
                        sig_rcv = generate_digital_signature(f"Engineering ({div_pemohon})", user_info["name"], item.get("nomor_opb", "OPB"))
                        item["status"] = "8. Selesai"
                        catat_log(item, f"Barang diverifikasi & diterima oleh Divisi {div_pemohon} ({user_info['name']}).", digital_sig=sig_rcv)
    ...
    # 2. ROLE PURCHASING
        ...
        tab1, tab2, tab3 = st.tabs(["1. Input Penawaran Harga", "2. Buat & Unggah IOM", "3. Serah Terima & Upload BAST"])
        ...
        with tab3:
            st.markdown("<div class='content-box'>", unsafe_allow_html=True)
            st.subheader("🤝 Serah Terima Barang & Upload Dokumen/Foto BAST")
            ...
            for item in items:
                is_expanded = (st.session_state["target_focus_id"] == item.get("id"))
                item_id = item.get("id", 0)
                div_pemohon = item.get("divisi", "Divisi Terkait")
                with st.expander(f"💳 {item.get('nomor_opb', '-')} - {item.get('nama_barang', '-')}", expanded=is_expanded):
                    st.write(f"**Divisi Pemohon:** {div_pemohon} | **Vendor:** {item.get('vendor', '-')}")
                    render_download_buttons(item, key_prefix=f"pur_tab3_{item_id}")

                    st.divider()
                    st.markdown("##### 📤 Unggah Berkas BAST & 3 Foto Dokumentasi")
                    file_bast = st.file_uploader("Upload Dokumen BAST (PDF/JPG/PNG)", type=["pdf", "jpg", "png"], key=f"bast_file_{item_id}")
                    
                    col_img1, col_img2, col_img3 = st.columns(3)
                    with col_img1:
                        foto_serah = st.file_uploader("📸 Photo Serah Terima", type=["jpg", "jpeg", "png"], key=f"f_serah_{item_id}")
                    with col_img2:
                        foto_proses = st.file_uploader("📸 Proses Pemasangan", type=["jpg", "jpeg", "png"], key=f"f_proses_{item_id}")
                    with col_img3:
                        foto_finishing = st.file_uploader("📸 Barang Terpasang (Finishing)", type=["jpg", "jpeg", "png"], key=f"f_finish_{item_id}")

                    st.info(f"ℹ️ **Info Tanda Tangan Digital:** Saat tombol di bawah diklik, sistem otomatis melampirkan tanda tangan digital resmi atas nama **Divisi {div_pemohon}** & **Purchasing**.")

                    if st.button("🚀 Serahkan Barang & Generate Tanda Tangan Otomatis", key=f"btn_p3_{item_id}", type="primary", use_container_width=True):
                        # Upload berkas & gambar jika ada
                        if file_bast:
                            item["file_bast_url"] = upload_file_to_supabase(file_bast.getvalue(), file_bast.name, folder="bast")
                        if foto_serah:
                            item["foto_serah_terima_url"] = upload_file_to_supabase(foto_serah.getvalue(), foto_serah.name, folder="dokumentasi")
                        if foto_proses:
                            item["foto_proses_pemasangan_url"] = upload_file_to_supabase(foto_proses.getvalue(), foto_proses.name, folder="dokumentasi")
                        if foto_finishing:
                            item["foto_finishing_url"] = upload_file_to_supabase(foto_finishing.getvalue(), foto_finishing.name, folder="dokumentasi")

                        # Generate tanda tangan digital otomatis sesuai divisi masing-masing
                        sig_handover = generate_digital_signature(f"Purchasing & Divisi {div_pemohon}", user_info["name"], item.get("nomor_opb", "OPB"))
                        item["status"] = "7. Verifikasi Penerimaan Barang (Engineering)"
                        catat_log(item, f"Purchasing menyerahkan fisik barang, BAST & dokumentasi foto ke Divisi {div_pemohon} dengan tanda tangan otomatis.", digital_sig=sig_handover)
                        save_database(item, is_new=False)
                        st.session_state["target_focus_id"] = None
                        st.toast("✅ Berhasil diserahterimakan dan tanda tangan otomatis diterapkan!", icon="🎉")
                        st.rerun()
            st.markdown("</div>", unsafe_allow_html=True)
